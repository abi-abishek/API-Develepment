...
class Signup
{
    ...
    public function sendVerificationMail()
    {
        ...
        $email->setFrom("abishekvlr@outlook.com", "API Test by Abiedits95");
        $email->setSubject("Verify your account");
        $email->addTo($this->email, $this->username);
        $email->addContent("text/plain", "Please verify your account at: https://api.selfmade.buzz/verify?token=$token");
        $email->addContent(
            "text/html",
            "<strong>Please verify your account by <a href=\"https://api.selfmade.buzz/verify?token=$token\">clicking here</a> or open this URL manually: <a href=\"https://api.selfmade.buzz/verify?token=$token\">https://api.selfmade.buzz/verify?token=$token</a></strong>"
        );
    ...
    {
        $query = "SELECT * FROM abishek_apidevcourse.auth WHERE token='$token';";
